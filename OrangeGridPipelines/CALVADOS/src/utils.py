...
def set_interactions(system, residues, prot, lj_eps, cutoff, yukawa_kappa, yukawa_eps, N, n_chains=1,
                     CoarseGrained="CA", dismatrix=None, isIDP=True, fdomains=None, chain_breaks=None):
    ...
    for j in range(n_chains):
        ...
        for i in range(begin, end - 1):  # index starts from 0
            if (i + 1) in chain_breaks:
                continue #no bond where protein breaks 
...
def simulate_simple(config):
    ...
    # parse config
    chain_breaks = set(config.get("chain_breaks") or [])
    ...
    hb, yu, ah = set_interactions(system, residues, prot, lj_eps, cutoff, yukawa_kappa, yukawa_eps, N,
                                  ...
                                  fdomains=fdomains, chain_breaks=chain_breaks)
    system.addForce(hb)
    system.addForce(yu)
    system.addForce(ah)
